  ...
  if (isLoading) return (
    <div className="grid grid-cols-1 gap-4 p-4 md:grid-cols-2 xl:grid-cols-4">
      {taskStatus.map((status) => (
        <div key={status} className="rounded-lg bg-gray-200 p-4 dark:bg-dark-secondary">
          <h3 className="mb-4 font-semibold dark:text-white">{status}</h3>
          <LoadingSkeleton count={3} height={100} className="mb-4" />
        </div>
      ))}
    </div>
  );
...
const PriorityTag = ({ priority }: { priority: TaskType["priority"] }) => {
  ...
  return (
    <div
      className={`rounded-full px-2 py-1 text-xs font-semibold ${getPriorityColor(priority)}`}
    >
      {priority}
    </div>
  );
